...
class Level3:
    ...
    def __init__(self, win, love, score, mute):
        ...
        self.enermys = [self.block1, self.enermy1, self.enermy2, self.enermy3, self.enermy4]
    ...
    def redrawWindow(self):
        ...
        text = Level3.font.render('LEVEL ' + str(self.level) + ' / ' + 'WAVE ' + str(self.wave + 1), 1, (0, 0, 0))
        self.win.blit(text, (500, 10))
        text = Level3.font.render(str(self.score), 1, (0, 0, 0))
        self.win.blit(text, (1230, 710))
        text = Level3.font.render(str(int(self.timeleft)), 1, (255, 0, 0))
        ...
        self.win.blit(Level3.hole, (0, 442))
    ...
    def RUN(self):
        while self.run:
            self.clocktime += Level3.clock.tick_busy_loop(45)
            ...
            if self.wave < 5:
                ...
                for i in range(1, len(self.enermys)):
                    if i==1:
                        ...
                        Dist = self.distance
                    if self.enermys[i].x > self.enermys[i-1].x + Dist:
                        self.enermys[i].left()
            ...
            if self.enermy1.health <= 0 and self.enermy2.health <= 0 and self.enermy3.health <= 0 \
                    and self.enermy4.health <= 0 and self.bomber.x == 173 and self.bomber.y == 335:
                if self.love > 0:
                    self.pausewin()
                if self.love <= 0:
                    self.pausedefeat()
